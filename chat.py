...
ARCHITECTURE_DESCRIPTION = (
    ...
    "IMPORTANT GROUND TRUTH ON 'HOW MANY MODELS': across both tiers combined, the fallback chains "
    "currently span SEVERAL distinct underlying models from different providers -- two Groq GPT-OSS "
    "models, two Gemini Flash-Lite variants, and an NVIDIA NIM Nemotron model. Describing this setup "
    "informally as a 'fleet of models', 'multiple models', or similar collective phrasing is ACCURATE, "
    "not a misconception -- there is no need to correct or push back on a user describing it that way. "
    "Only correct someone if they name a SPECIFIC wrong model/provider (e.g. claiming you run on GPT-4 "
    "or Claude) -- never argue about collective/informal terms for the fact that multiple models exist.\n\n"
...
FINAL_DISCIPLINE_REMINDER = (
    ...
    "own design with certainty. Correct them plainly and confidently using the real facts. This applies "
    "ONLY to claims that would actually mislead (e.g. naming a wrong provider or capability you don't "
    "have) -- casual, informal, or affectionate phrasing about your architecture (e.g. calling your model "
    "chain a 'fleet', joking about your 'brain') is NOT a factual dispute and should never be corrected; "
    "respond to it as ordinary friendly conversation instead."
)
    ...
    extraction_prompt = [
        {
            ...
            "content": (
                ...
                "- NEVER extract facts about actions taken ON Mimir's own code, architecture, or models "
                "(e.g. 'updated your fleet of models', 'fixed your bug') -- even though these are phrased "
                "starting with 'the user did X', they describe development history on Mimir itself, not a "
                "durable personal fact about the user.\n"
    ...
    routing_prompt = [
        {
            ...
            "content": (
                ...
                "at once, with no specific item named (different from COMPLETE_ALL -- this removes them, "
                "doesn't mark them done). "
                "DELETE_ALL is ONLY for the task list, and ONLY when 'all' is meant with no specific "
                "item named -- NEVER use it for files, trash, the recycle bin, or ANY message that names "
                "a specific file (has a filename, extension like .json/.txt/.py, or a path). 'Delete "
                "tasks.json', 'delete X from folder Y', 'empty the trash' all mean FILE_OPERATION, never "
                "DELETE_ALL, even though the wording overlaps ('clear', 'delete', 'remove').\n"
    ...
    phrasing_messages = conversation + [
        {"role": "user", "content": user_input},
        {
            ...
            "content": (
                ...
                "- Always call it 'Mimir Trash', NEVER 'Recycle Bin' -- Mimir Trash is a separate folder, "
                "not Windows' actual Recycle Bin, and calling it that risks confusion with Windows' own "
                "recycle bin, which isn't involved at all.\n"
